...
add_action( 'widgets_init', 'wpcasa_register_widget_spaces' );
 
function wpcasa_register_widget_spaces() {
	register_widget( 'wpCasa_Spaces' );
}
...
class wpCasa_Spaces extends WP_Widget {
	
	function __construct() {
	
		$widget_ops = array( 'description' => __( 'Content space with text and image', 'wpsight' ) );
		parent::__construct( 'wpCasa_Spaces', WPSIGHT_NAME . ' ' . __( 'Spaces', 'wpsight' ), $widget_ops );
		...
		if ( is_admin() ) {
			...
			if ( 'widgets.php' == $pagenow ) {
				...
				$data = array( 'wpcasa_change_image' => __( 'Change image', 'wpsight' ) );
				wp_localize_script( $this->id_base, 'wpcasa_spaces', $data );
				add_action( 'admin_head-widgets.php', array( $this, 'wpcasa_spaces_admin_head' ) );
	...
	function widget( $args, $instance ) {
		...
		if( $id == 'home' && $width == wpsight_get_span( 'full' ) )
	...
	function form( $instance ) {
		...
		<p>
			...
			<div id="display-<?php echo $this->get_field_id( 'image' ); ?>" class="wpcasa-spaces-image<?php if( empty( $instance['imageurl'] ) ) echo ' no-image'; ?>">
				<?php 
					if( $imageurl ) {
	...
	function is_wpcasa_spaces_widget_context() {
	...
	function replace_text_in_thickbox( $translated_text, $source_text, $domain ) {
		if ( $this->is_wpcasa_spaces_widget_context() ) {
	 ...
	 * Admin header css
	 */
	function wpcasa_spaces_admin_head() {
		?>
		<style type="text/css">
