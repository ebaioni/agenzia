...
add_action( 'widgets_init', 'wpsight_register_widget_slider' );
 
function wpsight_register_widget_slider() {

	if( current_theme_supports( 'FlexSlider' ) )
		register_widget( 'wpSight_Slider' );

}

class wpSight_Slider extends WP_Widget {
 
	function __construct() {
		$widget_ops = array( 'description' => __( 'jQuery content slider', 'wpsight' ) );
		parent::__construct( 'wpSight_Slider', ':: ' . WPSIGHT_NAME . ' ' . __( 'Slider', 'wpsight' ), $widget_ops );
    }
    ...
    function widget( $args, $instance ) {
		...
		if ( $slider->have_posts() ) {
			...
			<div id="<?php echo wpsight_dashes( $widget_id ); ?>" class="widget widget-slider clearfix">
				...
				<div class="widget-inner">
					...
					if( ! empty( $title ) ) { ?>
						...
						<div class="title title-widget clearfix">
						    ...
						    <?php
						    	echo apply_filters( 'wpsight_widget_slider_title', '<h2>' . $title . '</h2>', $args, $instance );
						    	do_action( 'wpsight_widget_slider_title_actions', $args, $instance );
						    ?>
					...
					$slider_args = apply_filters( 'wpsight_widget_slider_options_args', $slider_args, $args, $instance );
    ...
    function form( $instance ) {
		...
		<p>
			...
			<select class="widefat" id="<?php echo $this->get_field_id( 'filter' ); ?>" name="<?php echo $this->get_field_name( 'filter' ); ?>">
				<option value="" <?php selected( '', $filter ); ?>><?php _e( 'Latest posts', 'wpsight' ); ?></option>					
				<?php
		...
		<p<?php if( $instance['overlay'] == false ) echo ' style="display:none"'; ?>>
		    ...
		    <label for="<?php echo $this->get_field_id( 'teaser' ); ?>"><?php _e( 'Post teaser', 'wpsight' ); ?></label>		
		</p>
