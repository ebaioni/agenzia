...
if ( ! class_exists( 'rwmb_Image_Field' ) )
{
	class rwmb_Image_Field extends rwmb_File_Field
	{
		...
		static function wp_ajax_reorder_images()
		{
			...
			RW_Meta_Box::ajax_response( __( 'Order saved', 'wpsight' ), 'success' );
		}
		...
		{
			$i18n_title = _x( 'Upload images', 'image upload', 'wpsight' );
			$i18n_more  = _x( '+ Add new image', 'image upload', 'wpsight' );
		...
		{		
			$i18n_delete = _x( 'Delete', 'image upload', 'wpsight' );
			$i18n_edit   = _x( 'Edit', 'image upload', 'wpsight' );
			$li_bar = current_user_can( 'edit_post', $image ) ? '
