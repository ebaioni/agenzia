...
if ( ! class_exists( 'rwmb_Plupload_Image_Field' ) )
{
	class rwmb_Plupload_Image_Field extends rwmb_Image_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-plupload-image', 'rwmb', array( 'url' => WPSIGHT_METABOX_URL ) );
			wp_localize_script( 'rwmb-plupload-image', 'rwmb_plupload_defaults', array(
				...
				'filters'             => array(
					array(
						'title'      => _x( 'Allowed Image Files', 'image upload', 'wpsight' ),
		...
		static function html( $html, $meta, $field )
		{
			...
			if( isset( $post->ID ) ) {
				...
				$post_id = apply_filters( 'wpsight_plupload_post_id', $post->ID );
			...
			$i18n_drop   = apply_filters( 'rwmb_upload_drop_string', _x( 'Drop images here', 'image upload', 'wpsight' ) );
			$i18n_or     = _x( 'or', 'image upload', 'wpsight' );
			$i18n_select = _x( 'Select Files', 'image upload', 'wpsight' );
			$img_prefix  = $field['id'];
